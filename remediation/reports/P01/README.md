...
- **Classify:** `node remediation/reports/P01/classify.mjs` after every sweep run (the sweep
  deletes `raw/`). It writes `raw/CLASSIFIED.tsv` and exits 1 if any hit is unclassified.
...
| `07-enum-<Enum>` (×15) | each Prisma enum's members: quoted, `Enum.MEMBER`, or an object key                |  478 |
| `08-urls-domains`      | `duckdns`, `@spoh2027`, URLs, emails, AWS regions, ARNs, 12-digit IDs, Cognito IDs |  150 |
