...
## Time and time-zone audit (P01.7)

Instants are stored correctly: 58 of 59 `DateTime` columns are `@db.Timestamptz`, and the 59th is
`EventDay.date` (`@db.Date`, a calendar date). Capture timestamps are ISO instants from the device
(`new Date().toISOString()`), which is also correct. The problem is every place that turns an
instant into a local date, a local time or a bucket: they all assume UTC+8, and some assume UTC.

### Every wall-clock or calendar-date interpretation

| ID   | Where                                                                                                                                                                                                                                                                                                                                                                       | Assumes today                                                                                           | Must take instead                                                                                                                                          |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T-01 | `lib/time.ts:19,22` (`EVENT_TIME_ZONE`, `SGT_OFFSET_MINUTES`)                                                                                                                                                                                                                                                                                                               | one zone, fixed +8, no DST                                                                              | `Event.timeZone` (IANA), read through `platform/time` with a `Clock`                                                                                       |
| T-02 | "Today" via `singaporeDateString`: `middleware/rbac.ts:131`, `modules/announcement/service.ts:88`, `modules/attendance/service.ts:35,159,208,256`, `modules/dashboard/service.ts:49,207`, `modules/footfall/service.ts:55`, `modules/me/service.ts:32,103`, `modules/notification/service.ts:128`, `modules/registration/service.ts:41`, `modules/shift/service.ts:197,254` | local date = UTC instant + 8 h                                                                          | `eventLocalDate(instant, event)`, with an optional day-boundary hour for events that run past midnight                                                     |
| T-03 | "Which block is running" via `singaporeMinuteOfDay` + `activeShiftBlocks`: `middleware/rbac.ts:123`, `modules/attendance/service.ts:129`, `modules/dashboard/service.ts:54,142`, `modules/me/service.ts:33,104`, `modules/shift/service.ts:253`                                                                                                                             | minute of the local day compared with `HH:MM`; blocks never cross midnight                              | Each block's start and end resolved to instants for that event day in the event zone, compared as instants                                                 |
| T-04 | `lib/time.ts` `singaporeHourKey` (`modules/report/repo.ts:81`) and `singaporeHourStart` (no caller)                                                                                                                                                                                                                                                                         | a local hour label is unique                                                                            | Hour keys that carry the UTC offset, so the repeated hour on a DST day stays two buckets                                                                   |
| T-05 | `lib/time.ts` `floorToBucket`                                                                                                                                                                                                                                                                                                                                               | bucket edges on the UTC epoch                                                                           | Edges aligned to local midnight in the event zone                                                                                                          |
| T-06 | `lib/time.ts:82` `eventDayAnchor`; `modules/admin/service.ts:647`; `modules/roster/service.ts:205`                                                                                                                                                                                                                                                                          | a date string is a calendar date                                                                        | Stays. P09 adds "the date lies within the event's dates".                                                                                                  |
| T-07 | `modules/report/repo.ts:54,153,159,183`                                                                                                                                                                                                                                                                                                                                     | `AT TIME ZONE 'Asia/Singapore'`                                                                         | `AT TIME ZONE $1` with the event zone                                                                                                                      |
| T-08 | `modules/report/repo.ts:69`, `modules/registration/repo.ts:128,137`                                                                                                                                                                                                                                                                                                         | `date_trunc('hour' \| 'day', timestamptz)` in the **database session's** zone (UTC on a default server) | Explicit `AT TIME ZONE $1`. Today the day summary of registrations is a UTC day, which is right for Singapore only because the event runs 01:30–10:00 UTC. |
| T-09 | `modules/report/repo.ts:111` (30 min), `modules/footfall/repo.ts:103` (15/30/60 min)                                                                                                                                                                                                                                                                                        | epoch-aligned buckets                                                                                   | Local-aligned buckets (`date_bin` with an origin at local midnight). Wrong today for :30 and :45 offsets.                                                  |
| T-10 | `modules/report/export.ts:29–30`, headers `135,147,231,297,324,373,393,415`                                                                                                                                                                                                                                                                                                 | +8 h and the label `SGT`                                                                                | `Intl.DateTimeFormat` in the event zone; the header names the zone, and each cell carries its offset on DST days                                           |
| T-11 | `dto/settings.ts:24` `shiftBlocks` `HH:MM`; `dto/common.ts:25`                                                                                                                                                                                                                                                                                                              | `HH:MM` and dates are Singapore wall clock                                                              | "The event's zone"; block rows hold local times (P01.3)                                                                                                    |
| T-12 | `server/prisma/seed.ts:217–218,389`                                                                                                                                                                                                                                                                                                                                         | +8 h for "today" and for footfall                                                                       | The same event-zone helpers as the app                                                                                                                     |
| T-13 | `ops/backup/spoh-backup.sh:88–92`, `ops/backup/spoh-backup-check.sh:38–43`                                                                                                                                                                                                                                                                                                  | event hours are 01:30–10:00 UTC                                                                         | Superseded by RDS backups (P08.3), F01-028                                                                                                                 |
| T-14 | `client/src/lib/format.ts:14,21,24,33,38,45`                                                                                                                                                                                                                                                                                                                                | `timeZone: 'Asia/Singapore'`, locale `en-SG`                                                            | Event zone from the client's event context (P09.8); locale from the organisation or event                                                                  |
| T-15 | `client/src/lib/format.ts:59–72` `blockLabel`, `blockWord`                                                                                                                                                                                                                                                                                                                  | two fixed blocks with fixed hours                                                                       | Labels and hours from block rows (F01-046)                                                                                                                 |
| T-16 | `app/reports/page.tsx:72`                                                                                                                                                                                                                                                                                                                                                   | file name uses the **UTC** date                                                                         | The event-local date                                                                                                                                       |

`SHIFT_HOURS_ALWAYS_OPEN` (`lib/time.ts` `activeShiftBlocks`) and the purge, retention and duration
calculations (`minutesBetween`, `lostPersonPurgeHours`, `idempotencyRetentionDays`) use instants or
durations only and stay as they are.

### DST and offset hazards for a non-Singapore event

- **Blocks on a transition day.** On a spring-forward day a local time such as 02:30 does not exist,
  so a block that starts there opens at 03:00 and is an hour short. On a fall-back day 01:00–02:00
  happens twice, so a minute-of-day test (T-03) keeps a block running for an extra hour and the
  station scope open with it. The 13:30–14:00 handover overlap must still hold on those days.
- **Day boundaries.** A DST day has 23 or 25 hours. "Today" (T-02) and per-day report rows (T-07,
  T-08) must follow the zone, not a fixed offset. An event that runs past midnight needs an
  operational-day boundary; today's schema refuses a block whose end is before its start
  (`dto/settings.ts:37`).
- **Within event hours.** Station scope, attendance, the dashboard's running blocks, silent-station
  flags and the backup window all mean "during a running block". They inherit every T-03 hazard.
- **Buckets.** Hour buckets truncated in UTC are wrong for +05:30 and +09:30; 30-minute buckets on
  the UTC epoch are wrong for +05:45. On a fall-back day a local hour label collides with itself.
- **Labels.** An export column labelled with one abbreviation is wrong for half the rows around a
  transition (BST and GMT).

### DST and zone test cases (for P09.6)

Table-driven, with the clock frozen inside the event's zone as standard §9 requires.

| #   | Zone               | Case                                                           | Input (UTC)                                                       | Expected                                                                                                                                            |
| --- | ------------------ | -------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `Asia/Singapore`   | Regression: morning and afternoon both run during the handover | `2027-01-07T05:45:00Z` (13:45 local)                              | both blocks active; local date `2027-01-07`                                                                                                         |
| 2   | `Europe/London`    | Spring forward: block 00:30–02:30 local on 2027-03-28          | `00:59Z`, `01:00Z`, `01:30Z`                                      | active, active (02:00 BST), inactive (02:30 BST); block lasted 1 h                                                                                  |
| 3   | `Europe/London`    | Fall back: block 00:30–01:30 local on 2027-10-31               | `23:30Z` (30 Oct), `00:45Z`, `01:15Z`, `01:31Z`                   | active from 00:30 BST to 01:30 GMT, 2 h in total; inactive at `01:31Z` (an ambiguous end resolves to its later occurrence; P09.6 confirms the rule) |
| 4   | `Europe/London`    | Repeated hour stays two buckets                                | `2027-10-31T00:15:00Z`, `2027-10-31T01:15:00Z`                    | buckets `01:00+01:00` and `01:00+00:00`, one row each                                                                                               |
| 5   | `America/New_York` | Day boundary on a 23-hour day (2027-03-14)                     | `2027-03-15T03:59:00Z`, `2027-03-15T04:00:00Z`                    | local dates `2027-03-14` and `2027-03-15`                                                                                                           |
| 6   | `Asia/Kolkata`     | Hour bucket at +05:30                                          | `2027-01-07T04:10:00Z` (09:40 IST)                                | bucket starts `2027-01-07T03:30:00Z` (09:00 IST), not `04:00Z`                                                                                      |
| 7   | `Asia/Kathmandu`   | 30-minute bucket at +05:45                                     | `2027-01-07T04:20:00Z` (10:05 NPT)                                | bucket starts `2027-01-07T04:15:00Z` (10:00 NPT)                                                                                                    |
| 8   | `Australia/Sydney` | Southern-hemisphere summer time, date rollover                 | `2027-01-06T13:30:00Z`                                            | local date `2027-01-07` (00:30 AEDT, +11)                                                                                                           |
| 9   | `Europe/London`    | Export labels either side of a transition                      | `2027-10-30T12:00:00Z`, `2027-10-31T12:00:00Z`                    | `13:00 BST` (or `+01:00`) and `12:00 GMT` (or `+00:00`)                                                                                             |
| 10  | `Europe/Berlin`    | Overnight block 22:00–02:00 belongs to the day it starts       | `2027-06-12T23:30:00Z` (01:30 on 13 Jun, CEST)                    | block of event day `2027-06-12` active                                                                                                              |
| 11  | any                | `SHIFT_HOURS_ALWAYS_OPEN` in development                       | any instant                                                       | every block active                                                                                                                                  |
| 12  | any                | Database session zone is not UTC                               | set the session `TimeZone` to `Pacific/Auckland`, run the reports | per-day and per-hour rows unchanged                                                                                                                 |
