...
3. **If new:** call `identityProvider.ensureUser({ email, displayName, role })`, which:
   - In production/staging (Cognito): creates a Cognito user in the pool, adds them to the Cognito group matching their role (`ADMIN`→`Admin`, `LEAD`→`Lead`, etc.), and Cognito emails them an invite + temporary password.
   - In local dev: derives a deterministic fake `sub` from a SHA-256 of the email — no real account, no email sent.
4. **If already provisioned:** reuse the existing identity (`sub`) rather than minting a second one — re-provisioning (a role change, a phone-number correction) must not resend an invite.
5. Upsert the `Volunteer` row (`cognitoSub`, `displayName`, `email`, `phone`, `role`, `portfolio`, `reportsToId`).
